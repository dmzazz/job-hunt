    ...
    <>
      <div className="bg-slate-100 px-32 pt-10 pb-14">
        <div className="infline-flex gap-2 text-sm text-muted-foreground">
          ...
          / {""}
          <Link className="hover:underline hover:text-black" href={`/detail/company/${data?.Company?.CompanyOverview[0].id}`}>
            {data?.Company?.CompanyOverview[0].name}
          </Link>{" "}
          / {""}
          <Link className="hover:underline hover:text-black" href={`/detail/job/${data?.id}`}>
            {data?.roles}
          </Link>
        </div>
      ...
      <div className="px-32 py-16 flex flex-row items-start gap-10">
        <div className="w-3/4">
          ...
          <div className="mb-16">
            ...
            <div className="text-muted-foreground" dangerouslySetInnerHTML={{ __html: data?.responsibility!! }}></div>
          </div>

          <div className="mb-16">
            <div className="text-3xl font-semibold mb-3">Who You Are</div>
            <div className="text-muted-foreground" dangerouslySetInnerHTML={{ __html: data?.whoYouAre!! }}></div>
          </div>

          <div className="mb-16">
            <div className="text-3xl font-semibold mb-3">Nice-To-Haves</div>
            <div className="text-muted-foreground" dangerouslySetInnerHTML={{ __html: data?.niceToHaves!! }}></div>
          </div>
        </div>
        <div className="w-1/4">
          <div>
            ...
            <div className="mt-6 p-4 bg-gray-50">
              <div className="mb-2">
                <span className="font-semibold">{data?.applicants} Applied</span> <span className="text-gray-600">of {data?.needs} capacity</span>
              </div>
      ...
      <div className="px-32 pb-16">
        ...
        <div className="mb-6">
          ...
          <div className="text-gray-500 mt-1">This job comes with several perks and benefits</div>
        </div>
